      ...
      return JSON.stringify(
        {
          ...
          usage:
            'Call enable_tools with group names to load more tools. Groups: mdms (tenant validation + MDMS CRUD), boundary (boundary hierarchy + boundary management), masters (departments, designations, complaint types), employees (HRMS employee create/update/validate), localization (UI labels), pgr (complaints + workflow), admin (filestore upload/download + access control + user search/create), idgen (ID generation), location (geographic boundaries), encryption (encrypt/decrypt data).',
        },
        null,
        2
      );
